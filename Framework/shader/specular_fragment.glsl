...
{                                                                                           
    ...
    return (AmbientColor * (1.0 - AO / 2.0 * gShowAO) +  ShadowFactor * (DiffuseColor + SpecularColor));                                  
} 
...
if(gShowShadow == 1)
{
   ...
   TotalLight = CalcDirectionalLight(Normal, ShadowFactor);                                        
 }
  else
  {
   TotalLight = CalcDirectionalLight(Normal, 1.0); 
  }
	...
	color = color * TotalLight + CascadeIndicator;
  }
  else
  {
   color = vec4(scale0,scale0,scale0,1.0);
  }
}