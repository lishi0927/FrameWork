...
void Shader_Phong::SetWVP(const glm::mat4 &WVP)
{
	glUniformMatrix4fv(m_WVPLocation, 1, GL_FALSE, &WVP[0][0]);
}

void Shader_Phong::SetWorldMatrix(const glm::mat4 &WorldMatrix)
{
	glUniformMatrix4fv(m_WorldMatrixLocation, 1, GL_FALSE, &WorldMatrix[0][0]);
}

void Shader_Phong::SetDirectionalLight(const DirectionalLight& Light)
{
...
void Shader_Phong::SetEyeWorldPos(const glm::vec3& EyeWorldPos)
{
	glUniform3f(m_eyeWorldPosLocation, EyeWorldPos.x, EyeWorldPos.y, EyeWorldPos.z);
}

void Shader_Phong::SetMatSpecularIntensity(float Intensity)
{
	glUniform1f(m_matSpecularIntensityLocation, Intensity);
}

void Shader_Phong::SetMatSpecularPower(float Power)
{
	glUniform1f(m_matSpecularPowerLocation, Power);
}

void Shader_Phong::SetPointLights(unsigned int NumLights, const PointLight* pLights)
{
...
void Shader_Phong::SetSpotLights(unsigned int NumLights, const SpotLight* pLights)
{
...
void Shader_Phong::SetScale(const glm::vec4 &scale)
{
	glUniform4f(m_scaleLocation, scale.x, scale.y, scale.z, scale.w);
}

void Shader_Phong::SetOffset(const glm::vec4 &offset)
{
	glUniform4f(m_offsetLocation, offset.x, offset.y, offset.z, offset.w);
}

void Shader_Phong::SetCurrentPosition(const glm::vec4 &currentPos)
{
	glUniform4f(m_currentPosLocation, currentPos.x, currentPos.y, currentPos.z, currentPos.w);
}

void Shader_Phong::SetHeightmap(unsigned int TextureUnit)
{
	glUniform1i(m_heightmapLocation, TextureUnit);
}

void Shader_Phong::SetVtex(unsigned int TextureUnit)
{
	glUniform1i(m_vtexLocation, TextureUnit);
}

void Shader_Phong::SetBtex(unsigned int TextureUnit)
{
	glUniform1i(m_bexLocation, TextureUnit);
}

void Shader_Phong::SetBlendtex(int index, unsigned int TextureUnit)
{
	glUniform1i(m_bexBlendlocation[index], TextureUnit);
}

void Shader_Phong::SetColortex(int index, unsigned int TextureUnit)
{
