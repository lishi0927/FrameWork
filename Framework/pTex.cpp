...
void PTex::init(bool isHeightmap, bool isAOmap, string dataname) {
	if (isHeightmap || isAOmap)
	...
	if (isHeightmap || isAOmap)
		glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, pixelSize, pixelSize,
...
void PTex::update(int level, int x, int y, int px, int py, int vw, int vh) {
	...
	for (int i = 0; i < 9; i++) {
		...
		if (isHeightmap) {
			terrainData.loadHeightmap(tx, ty, i);
		}
		else if (isAOmap)
		{
			terrainData.loadAOmap(tx, ty, i);
		}
		else {
	...
	if (isHeightmap) {
		...
		glTexSubImage2D(GL_TEXTURE_2D, 0, px * pageSize, py * pageSize,
			...
			GL_RED, GL_FLOAT, terrainData.getHeightmap());
		glBindTexture(GL_TEXTURE_2D, 0);
//		terrainData.calNormal();
	...
	else if (isAOmap)
	{
		...
		glTexSubImage2D(GL_TEXTURE_2D, 0, px * pageSize, py * pageSize,
			terrainData.getWidth(), terrainData.getHeight(),
			GL_RED, GL_FLOAT, terrainData.getAOmap());
		glBindTexture(GL_TEXTURE_2D, 0);
	}
}
